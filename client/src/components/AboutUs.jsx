...
const AboutUs = () => {
  ...
  `}</style>
      ...
      {/* WHAT WE DO - Sharp Glow Professional Design */}
    ...
    .what-we-do-section .glow-card-content {
      background: rgba(200, 194, 194, 0.8);
  ...
  `}</style>
            ...
            <Col key={idx} xs={12} sm={6} lg={4}>
              ...
              >
                <div className="neon-inner">
                  ...
                  <h5 className="neon-title">{card.title}</h5>
                  <p className="neon-desc">{card.desc}</p>
                </div>
              </motion.div>
            </Col>
          ))}
    ...
    .neon-card::before {
      ...
      border-radius: 18px;
      padding: 2px;
      background: linear-gradient(120deg, #00a8ff, #7b2ff7, #ff4081, #00a8ff);
      background-size: 300% 300%;
    ...
    @media (max-width: 768px) {
      .neon-inner { padding: 25px 18px; }
      .neon-icon { width: 60px; height: 60px; font-size: 1.8rem; }
    }
  `}</style>
      </Container> */}
